...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function onPointerEnd(x) {
        if (!pointerDown || startX === null) return (pointerDown = false);
        const diff = x - startX;
        const threshold = 50;
...
function renderChartLines() {
    ...
    barsContainer.querySelectorAll(".chart__line-container").forEach((l) => l.remove());
    ...
    const totalSteps = maxValue / stepValue;

    const stepWidth = (containerWidth / (maxValue + stepValue)) * stepValue;
    ...
    for (let i = 0; i <= totalSteps; i++) {
      ...
      <span class="chart__line-number">${i * stepValue}</span>
    `;
...
function updateChart(year, quarter) {
    window.__lastChartYear = year;
    window.__lastChartQuarter = quarter;
    if (!chartData[year]) return;
            ...
            if (groupData[i]) {
                ...
                setTimeout(() => {
                    ...
                    requestAnimationFrame(() => {
                        bar.style.width = (barsContainer.clientWidth / 500) * d.value + "px";
                    });
                }, 50);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const barsContainerElement =
        barsContainer ||
        document.querySelector(".chart__bars") ||
        document.querySelector(".bars-container") ||
        document.querySelector(".chart-container");

    let __chartResizeTimeout = null;

    function scheduleChartUpdate() {
        clearTimeout(__chartResizeTimeout);
        __chartResizeTimeout = setTimeout(() => {
            const y = window.__lastChartYear || defaultYear;
            const q = window.__lastChartQuarter || defaultQuarter;
            updateChart(y, q);
        }, 80);
    }

    if (barsContainerElement && window.ResizeObserver) {
        const ro = new ResizeObserver(() => scheduleChartUpdate());
        ro.observe(barsContainerElement);
    }

    window.addEventListener("resize", scheduleChartUpdate);
...
yearDropdown.style.cssText = `
  position: absolute; background: #fff; border: 0.1rem solid #ccc;
  list-style: none; padding: 0; display: none; border-radius: 1rem;
  z-index: 999;
`;
const style = document.createElement("style");
style.innerHTML = `
  .chart__year-dropdown li:hover {
    background-color: #f0f0f0; cursor: pointer; border-radius: 1rem;
  }
`;
document.head.appendChild(style);
parent.appendChild(yearDropdown);
...
yearSelector.addEventListener("click", () => {
    const visible = yearDropdown.style.display === "block";
    if (!visible) positionYearDropdown();
    yearDropdown.style.display = visible ? "none" : "block";
});
...
(() => {
    ...
    function updateScrollVar() {
        ...
        document.body.style.setProperty("--scroll-y", window.scrollY || 0);
        ticking = false;
    }
